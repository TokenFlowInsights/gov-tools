...
for voter in voters:
    ...
    '''
    # MKRs on hot wallet
    MKRs_HOT = balance_of(chain, MKR, voter, token_abi, poll_closing)
    ...
    IOU_1_0 = balance_of(
        chain, IOU_1_0_address, voter, token_abi, poll_closing
    )

    # MKRs staked directly to DSChief 1.1
    IOU_1_1 = balance_of(
        chain, IOU_1_1_address, voter, token_abi, poll_closing
    )

    # MKRs staked directly to DSChief 1.2
    IOU_1_2 = balance_of(
        chain, IOU_1_2_address, voter, token_abi, poll_closing
    )

    """
    STAKES VIA VoteProxy; COLD STORAGE
    """
    if has_proxy(chain, voter, vote_proxy_factory_V1_address, vote_proxy_factory_V1_abi, poll_closing):
        ...
        ).call(block_identifier=poll_closing)
        # MKRs staked to DSChief 1.1 via VoteProxy
        STAKED_VIA_PROXY_1_1 = balance_of(chain, IOU_1_1_address, vote_proxy, token_abi, poll_closing)
        ...
        cold = vote_proxy_contract.functions.cold().call(block_identifier=poll_closing)
        if cold.lower() != voter.lower():
            MKRs_1_1_COLD = balance_of(chain, MKR, cold, token_abi, poll_closing)

    if has_proxy(chain, voter, vote_proxy_factory_V2_address, vote_proxy_factory_V2_abi, poll_closing):
        ...
        STAKED_VIA_PROXY_1_2 = balance_of(chain, IOU_1_2_address, vote_proxy, token_abi, poll_closing)
        ...
        cold = vote_proxy_contract.functions.cold().call(block_identifier=poll_closing)
        if cold.lower() != voter.lower():
            MKRs_1_2_COLD = balance_of(chain, MKR, cold, token_abi, poll_closing)
...
print({"VOTERS": len(voters), "POLL CLOSING BLOCK": poll_closing, "VOTING POWER": VOTING_POWER})
